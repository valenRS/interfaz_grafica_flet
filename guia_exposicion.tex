...
\begin{itemize}
    ...
    \item \textbf{utils/api_client.py}: consulta Open‑Meteo y traduce las claves JSON a español.
    \item \textbf{utils/data_manager.py}: lectura y escritura de los CSV en \texttt{data/}.
    \item \textbf{utils/settings.py}: preferencias de unidades (°C/°F, km/h/mph).
    \item \textbf{utils/chart_generator.py}: genera las gráficas PNG para \texttt{assets/charts/}.
    \item \textbf{views/}: contiene las pantallas de la app; \texttt{home_view.py} es la principal.
...
\subsection{Instrucciones r\'apidas para ejecutar la app}
Usa un entorno virtual y luego instala dependencias. Instrucciones para Linux/macOS y para Windows (PowerShell o cmd):
...
\noindent\textbf{Nota técnica:} si ejecutas en entorno sin GUI (servidor), aseg\'urate de que \texttt{utils/chart_generator.py} use el backend \texttt{Agg} de matplotlib (sin interfaz) para generar PNG.
...
El archivo \texttt{main.py} es el \textbf{corazón de la navegación}. Cuando se ejecuta \texttt{python main.py}, ocurre lo siguiente:
